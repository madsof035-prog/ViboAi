...
// دالة لفك تشفير الإعدادات من base64
const decodeSettingsFromBase64 = (encodedSettings) => {
    try {
        const decodedString = decodeURIComponent(escape(atob(encodedSettings)));
        return JSON.parse(decodedString);
    } catch (error) {
        console.error('خطأ في فك تشفير الإعدادات:', error);
        return null;
    }
};

// دالة لتحميل الإعدادات المخصصة للبوت - محدثة للمشاركة
const loadCustomBotSettings = () => {
    const urlParams = new URLSearchParams(window.location.search);
    const botId = urlParams.get('bot');
    const encodedConfig = urlParams.get('config');
    
    if (botId && encodedConfig) {
        // تحميل الإعدادات من URL parameters (للروابط المشتركة)
        const decodedSettings = decodeSettingsFromBase64(encodedConfig);
        if (decodedSettings) {
            console.log('تم تحميل إعدادات البوت المشترك:', decodedSettings);
            return decodedSettings;
        }
    } else if (botId) {
        // تحميل الإعدادات من localStorage (للاستخدام المحلي)
        const savedSettings = localStorage.getItem('botCustomSettings');
        if (savedSettings) {
            try {
                return JSON.parse(savedSettings);
            } catch (error) {
                console.error('خطأ في تحميل إعدادات البوت من localStorage:', error);
            }
        }
    }
    return null;
...
مهم جداً: لا تذكر أبداً أنك تستخدم معلومات من إعدادات مخصصة، تصرف وكأن هذه هي طبيعتك الأساسية.
`;
};
...
`;
    ...
    if (customSettings) {
      ...
      console.log("🤖 تم تحميل إعدادات البوت المخصص:", customSettings);
    } else {
      systemInstructions = `
